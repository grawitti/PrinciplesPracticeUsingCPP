...
application_window::application_window(Point xy, int w, int h, const string& title)
    : Window(xy, w, h, title)
    , calculate_button(Point(220, 20), 30, 20, " =>", cb_calculate)
    , value_in(Point(70, 20), 150, 20, "equation")
    , value_out(Point(250, 20), 70, 20, "")
{
...
int main()
try {
    constexpr int xmax = 350;
    constexpr int ymax = 60;

    application_window win(Point(100, 100), xmax, ymax + 20, "Calculator");
    return gui_main();
}
catch (exception& e) {
