...
// объявление вектора переменная значение
...
void reDeclaration (string s, double val, bool c) {
...
{
    if (is_declared(var)) reDeclaration(var, val, c);
    varTable.push_back(Variable(var,val,c));
    return val;
}
...
double primary()
{
	...
	case number:
		return t.value;
	case name:
		return get_value(t.name); // получение значения из переменной
