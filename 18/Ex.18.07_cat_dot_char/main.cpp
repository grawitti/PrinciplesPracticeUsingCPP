...
string cat_dot(const string& s1, const string& s2, const char& d)
{
	return s1+d+s2;
}

// cat_dot with char* s1 and s2
char* cat_dot_char(const char* s1, const char* s2, const char& d)
{
	char* result = new char;
	int i = 0;
	while(*s1) // paste s1 in result
	{
		*result=*s1;
		result++;
		s1++;
...
int main()
{
    try
    {
		...
		char ss1[] = "One_char";
		char ss2[] = "Two_char";
		char* sss = cat_dot_char(ss1,ss2,',');
		print_char_array(cout,sss);
		delete[] sss;
		// delete[] s2;
