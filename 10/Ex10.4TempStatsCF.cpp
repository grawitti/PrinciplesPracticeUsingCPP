/**
 ...
 Автор: Руслан
 Назначение: Считывает показания температуры из файла (по Цельсию - с или Фарингейту - f), вычисляет среднюю и медиану температур.
...
// Перевод температуры из шкалы Фаренгейта в шкалу Цельсия = F-32 * (5/9)
double farenToC (const double& f)
{
	return (f-32.0) * (5/9.0);
}
...
void readFileTemps (const string& fileName, vector<Reading>& vr)
{
	...
	while (!ifs.eof()) 
	{
		...
		ifs >> ch0 >> ch1 >> hour >> ch2 >> temperature >> ch3;
		...
		if (ch0 == 'c') vr.push_back(Reading{hour,temperature});
		else if (ch0 == 'f') vr.push_back(Reading{hour,farenToC(temperature)});
		else error("Не поддерживаемая шкала температур: ", ch0);
	}
}
